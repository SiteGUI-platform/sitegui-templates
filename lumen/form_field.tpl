{function buildForm}
  ...
  {else} 
    ...
    <div class="col-sm-{$c2|default:7}">
      {if $field.type == 'text' || $field.type == 'password' || $field.type == 'url' || $field.type == 'email' || $field.type == 'tel' || $field.type == 'color'}
      ...
      {elseif $field.type == 'lookup'}
        ...
        {if $field.visibility != readonly || !$field.value}
          ...
          <input class="form-control lookup-field dropdown-toggle rounded-end" type='text' placeholder="{'Name'|trans}" data-lookup="{$field.lookup_key|default:$key}" data-index="{$form_script_loaded}" {if $field.options.scope}data-scope="{$field.options.scope}"{/if} data-name="{$fieldPrefix}[{$key}]{if $field.is == multiple}[ ]" data-multiple="1{/if}" data-bs-auto-close="outside" data-bs-toggle="dropdown" aria-expanded="false">
