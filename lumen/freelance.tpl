    ...
    <div class="col-12 px-0">
        ...
        <div class="row">
                    ...
                    <ul id="main-tab" class="nav nav-tabs px-3" role="tablist">
                        {foreach $api.subapp AS $name => $subapp}
                            {if ! $subapp.hide.tabapp }
                              <li class="nav-item">
                                <button type="button" class="nav-link position-relative {if $subapp@first}active{/if}" data-bs-toggle="tab" data-bs-target="#app-{$name}-tabpane" role="tab" aria-controls="app-{$name}-tabpane" aria-selected="false">{$name|replace: '_':' '|trans}</button>
                              </li> 
                            {/if}
                        {/foreach}
                    </ul>
                                       ...
                                       ($api.app.sub.$name.entry == other_readonly AND $api.page.creator == $user.id) 
                                        ...
                                        <form action="{$links.update}" method="post" enctype="multipart/form-data">
                                            <div class="text-start pb-3">
                                              <button class="btn btn-outline-primary" type="button" data-bs-toggle="collapse" data-bs-target="#collapse-sub-{$name}" aria-expanded="false" aria-controls="collapse-sub-{$name}">{"New :item"|trans:["item" => $name]}</button>
                                            </div>  
            ...
            <div class="col-md order-first order-md-last mx-md-3">
                <div class="sticky-sm-top pt-sm-4">
                    ...
                    {foreach $filters AS $key => $filter}
                        ...
                        {foreach $filter AS $opt => $class}
                            ...
                            <button 
                                ...
                                data-select="{$select}" 
                                class="btn btn-sm border-1 border-dark my-2 position-relative {if $class@first}active btn-outline-warning{/if} {$class|replace:$select:''}" 
                                {if $filter@first}
                                    style="padding: 2px;">
                    ...
                    <form action="{$links.cart_add}" method="POST">
                        ...
                        <input type="hidden" name="csrf_token" value="{$token}">
                        <button type="submit" class="btn btn-sm btn-success my-3 me-3" id="js-cart-add">{'Make Order'|trans}</button>
